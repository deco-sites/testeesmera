...
try {
  ...
  const cssHref = await desktop.locator('link[href*="esmera-product-media-v16.css"]').getAttribute("href");
  if (!cssHref?.includes("2026-08-11-product-media-fidelity-v16")) {
    fail("Media fidelity stylesheet revision is stale", { cssHref });
  }

  await openFixture(desktop, fixtures.singleLandscape);
  const singleClass = await desktop.locator(".esv-product-modal-gallery").getAttribute("class");
  ...
  const splitPresentation = await desktop.locator(".esv-product-modal-gallery-frame").getAttribute("data-two-image-presentation");
  ...
  await recordScenario("desktop-two-portrait-split", { presentation: splitPresentation });

  await openFixture(desktop, fixtures.twoMixed);
  const mixedPresentation = await desktop.locator(".esv-product-modal-gallery-frame").getAttribute("data-two-image-presentation");
  ...
  await desktop.getByRole("button", { name: "Próxima imagem da galeria" }).click();
  ...
  await recordScenario("desktop-two-mixed-stage", { presentation: mixedPresentation });
